...
module "windows_vm_file" {
    source = "./modules/7-windows_fileserver"
    computer_name_Windows = "${var.computer_name_Windows_FS}"
    rg_network = "${azurerm_resource_group.rg_network.name}"
    ...
    private_ip_address = "${var.ip_address-FS01}"
    dns_prefix = "${var.dns_prefix}"
}

// Create Windows VM FILESERVER
